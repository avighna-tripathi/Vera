...
def _build_refinement_prompt(resolved: ResolvedContexts, draft_body: str, draft_cta: str, draft_rationale: str) -> dict[str, Any]:
    ...
    return {
        ...
        "rules": [
            ...
            "No URLs.",
            "Keep the message concise and natural: 2-4 short sentences, under 90 words.",
            "Retain at least two concrete facts from the trigger or merchant context when available (for example timing, metric, offer, service, location, customer name, or deadline).",
            "State why this message is useful now, then end with exactly one simple, answerable CTA.",
            "For customer messages, write as the merchant and never mention magicpin or internal trigger data.",
